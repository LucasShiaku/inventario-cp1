...
public class Main {
    ...
    public static void saveItem(EntityManager manager) {
        ...
        tipoDeBem.setNome("X6 BM");

        Departamento departamento = new Departamento();
        departamento.setNome("AUTOMOTIVEL");

        Inventario inventario = new Inventario();
        inventario.setDepartamento(departamento).setInicio(LocalDate.now());

        Bem bem = new Bem();
        bem.setNome("CARRO").setTipo(tipoDeBem).setEtiqueta("01").setLocalizacao(departamento).setAquisicao(LocalDate.now());
